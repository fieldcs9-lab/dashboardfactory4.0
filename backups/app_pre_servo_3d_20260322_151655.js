/**
 * J.A.R.V.I.S. HUD Core v5.1.0
 * Feature: 3D Holographic Machine Visualization (Three.js)
 */

let currentZone = 'ALL';
let lastReceivedData = null;
const threeScenes = {}; // Map to store Three.js instances per machine
...
function init3D(containerId, status) {
    ...
    // Create Holographic Machine Model (Complex Wireframe)
    const geometry = new THREE.BoxGeometry(1, 1, 1);
    const edges = new THREE.EdgesGeometry(geometry);
    
    let color = 0x38bdf8; // Default Cyan
    if (status === 'RUNNING') color = 0x34d399;
    if (status === 'IDLE') color = 0xfbbf24;
    if (status === 'STOP') color = 0xf87171;

    const lineMaterial = new THREE.LineBasicMaterial({ color: color, linewidth: 2 });
    const wireframe = new THREE.LineSegments(edges, lineMaterial);
    
    // Add inner glow-like cube
    const innerGeom = new THREE.BoxGeometry(0.7, 0.7, 0.7);
    ...
    function animate() {
        ...
        machineGroup.rotation.x += 0.005;
        
        // Dynamic speed based on status
        if (status === 'RUNNING') machineGroup.rotation.y += 0.02;
        if (status === 'STOP') {
    ...
    // Store reference for color updates if needed
    threeScenes[containerId] = { scene, wireframe, innerCube, colorVar: color };
}
...
function renderDashboard(data) {
    ...
    // Initialize 3D viewports after rendering HTML
    filtered.forEach(m => {
