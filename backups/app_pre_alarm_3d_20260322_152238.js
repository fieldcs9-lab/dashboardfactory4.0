/**
 * J.A.R.V.I.S. HUD Core v5.2.0
 * Feature: 3D Holographic Servo Motor Simulation
 */
...
// === THREE.JS 3D ENGINE (SERVO MOTOR) ===
function init3D(containerId, status) {
    ...
    let color = 0x38bdf8; // Sky Blue (JARVIS)
    if (status === 'RUNNING') color = 0x34d399; // Emerald
    if (status === 'IDLE') color = 0xfbbf24;    // Amber
    if (status === 'STOP') color = 0xf87171;    // Coral

    const material = new THREE.LineBasicMaterial({ color: color, transparent: true, opacity: 0.8 });
    const innerMaterial = new THREE.MeshBasicMaterial({ color: color, transparent: true, opacity: 0.05 });

    const motorGroup = new THREE.Group();

    // 1. Motor Body (Rectangular Box)
    const bodyGeom = new THREE.BoxGeometry(0.8, 0.8, 1.2);
    const bodyEdges = new THREE.EdgesGeometry(bodyGeom);
    const bodyWire = new THREE.LineSegments(bodyEdges, material);
    const bodyMesh = new THREE.Mesh(bodyGeom, innerMaterial);
    motorGroup.add(bodyWire);
    motorGroup.add(bodyMesh);

    // 2. Front Flange (Square Plate)
    const flangeGeom = new THREE.BoxGeometry(1.0, 1.0, 0.1);
    const flangeEdges = new THREE.EdgesGeometry(flangeGeom);
    const flangeWire = new THREE.LineSegments(flangeEdges, material);
    flangeWire.position.z = 0.65;
    motorGroup.add(flangeWire);

    // 3. Rotating Shaft (Cylinder)
    const shaftGeom = new THREE.CylinderGeometry(0.15, 0.15, 0.6, 8);
    const shaftEdges = new THREE.EdgesGeometry(shaftGeom);
    const shaftWire = new THREE.LineSegments(shaftEdges, material);
    
    // Rotate shaft geometry to align with Z axis
    shaftWire.rotation.x = Math.PI / 2;
    shaftWire.position.z = 0.95;
    
    const shaftGroup = new THREE.Group();
    shaftGroup.add(shaftWire);
    motorGroup.add(shaftGroup);

    scene.add(motorGroup);

    // Initial View Position
    motorGroup.rotation.y = -Math.PI / 4;
    motorGroup.rotation.x = Math.PI / 8;
    ...
    function animate() {
        ...
        if (status === 'RUNNING') {
            // Fast rotation for shaft
            shaftGroup.rotation.z += 0.15;
            // Subtle vibration for the whole motor
            motorGroup.position.x = Math.sin(time * 50) * 0.01;
            motorGroup.position.y = Math.cos(time * 50) * 0.01;
        } else if (status === 'IDLE') {
            // Slow standby rotation
            shaftGroup.rotation.z += 0.02;
        ...
        // Slow hover rotation for the whole group
        motorGroup.rotation.y += 0.005;
...
function renderOverview(container, machines) {
    ...
    container.innerHTML += `
        <div class="overview-grid">
            ...
            <div class="stat-card" style="border-top: 3px solid var(--accent-running)">${ICONS.RUNNING}<span class="stat-label">STATUS_ACTIVE</span><span class="stat-value" style="color: var(--accent-running)">${running}</span></div>
            <div class="stat-card" style="border-top: 3px solid var(--accent-idle)">${ICONS.IDLE}<span class="stat-label">STATUS_IDLE</span><span class="stat-value" style="color: var(--accent-idle)">${idle}</span></div>
            <div class="stat-card" style="border-top: 3px solid var(--accent-stop)">${ICONS.STOP}<span class="stat-label">STATUS_OFFLINE</span><span class="stat-value" style="color: var(--accent-stop)">${stop}</span></div>
        </div>
    `;
}
    ...
    machines.forEach(m => {
        ...
        cards += `
            <div class="machine-card status-${s}">
                ...
                <div id="3d-${m.id}" class="machine-3d-viewport"></div>
